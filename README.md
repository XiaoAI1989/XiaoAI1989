## Hi, I'm Zhiyuan Wang 👋

·M.S. in AI, Tsinghua University

I work on **LLM reasoning & decision-making, continual learning, and on-device multimodal agents** .

---

### 🔬 Research Interests
- **On-device multimodal LLM / GUI Agent**: cloud-to-edge distillation, on-device SFT/DPO, FSM-constrained decoding, edge quantization
- **LLM reasoning & decision-making**: multi-agent collaboration, operations-research solving, Decision Transformers
- **Continual / prompt learning**: rehearsal-free class-incremental learning, cross-modal prompt synergy

### 📄 Selected Publications (first author)
| Paper | Venue | Links |
|---|---|---|
| **ORMind**: A Cognitive-Inspired End-to-End Reasoning Framework for Operations Research | ACL 2025 (Industry), CCF-A | [Paper](https://aclanthology.org/2025.acl-industry.10) |
| **ChordPrompt**: Cross-Modal Prompt Synergy for Multi-Domain Incremental Learning in CLIP | ECML-PKDD 2025, CCF-B | [Paper](https://link.springer.com/chapter/10.1007/978-3-662-72243-5_9) |
| **P2DT**: Mitigating Forgetting in Task-Incremental Learning with Progressive Prompt Decision Transformer | ICASSP 2024, CCF-B | [Paper](https://ieeexplore.ieee.org/document/10447725) · [Code](#) |
| **INCPrompt**: Task-Aware Incremental Prompting for Rehearsal-Free Class-Incremental Learning | ICASSP 2024, CCF-B | [Paper](https://ieeexplore.ieee.org/document/10447775) · [Code](#) |
| **Partial Task Offloading Strategy Based on Deep Reinforcement Learning** | IEEE ICCC 2020 | [Paper](https://ieeexplore.ieee.org/document/9345003) |

> 📚 Full list on [Google Scholar](https://scholar.google.com/citations?hl=en&user=ms3k-RkAAAAJ)

### 🛠️ Tech Stack
`Python` · `PyTorch / Transformers` · `DeepSpeed / Accelerate` · `LangChain / LangGraph` · `SFT / DPO / LoRA` · `CLIP / ViT`

### 📫 Contact
[![Email](https://img.shields.io/badge/Email-834006701%40qq.com-blue?style=flat-square&logo=gmail)](mailto:834006701@qq.com)
[![Scholar](https://img.shields.io/badge/Google%20Scholar-4285F4?style=flat-square&logo=googlescholar&logoColor=white)](https://scholar.google.com/citations?hl=en&user=ms3k-RkAAAAJ)
